...
class Bot(asynchat.async_chat):
    ...
    def log_raw(self, line, prefix):
        ...
        f = codecs.open(os.path.join(self.config.core.logdir, 'raw.log'),
                        'a', encoding=self.config.core.encoding, errors="replace")
    ...
    def write(self, args, text=None):
        ...
        try:
            ...
            if text is not None:
                ...
                temp = ' '.join(args)[:510] + '\r\n'
            self.log_raw(temp, '>>')
            encoding = self.config.core.encoding
            self.send(temp.encode(encoding, 'ignore'))
        finally:
    ...
    def collect_incoming_data(self, data):
        ...
        try:
            data = unicode(data, encoding=self.config.core.encoding)
